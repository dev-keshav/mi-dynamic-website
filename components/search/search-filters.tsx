...
import { countries, dataTypes } from "@/constants"
...
export default function SearchFilters(type: any) {
  ...
  // console.log("dataTypes------------->", router)

  // const countrycode = getCountryCodeByName(country)
  // const states = countrycode && State.getStatesOfCountry(countrycode);
  useEffect(() => {
    if (selectedcountry) {
      const currstates: IStateDetail[] = State.getStatesOfCountry(
        selectedcountry
      ).map((state) => ({
        name: state.name,
  ...
  const [destinationCountryCounts, setDestinationCountryCounts] = useState<{ [key: string]: number }>(
    {}
  )
  ...
  //       countryFrequency[product.origin_country] =
  //         (countryFrequency[product.origin_country] || 0) + 1
  //     })
  ...
  useEffect(() => {
    if (trade && trade.products) {
      const originCountryFrequency: { [key: string]: any } = { All: "" }
      const destinationCountryFrequency: { [key: string]: any } = { All: "" }
  
      trade.products.forEach((product: Product) => {
        originCountryFrequency[product.origin_country] =
          (originCountryFrequency[product.origin_country] || 0) + 1;
  
        destinationCountryFrequency[product.destination_country] =
          (destinationCountryFrequency[product.destination_country] || 0) + 1;
      });
  
      setCountryCounts(originCountryFrequency);
      setDestinationCountryCounts(destinationCountryFrequency);
    }
  }, [trade]);
  

  // console.log("CountryCOunt---->", countryCounts)
  ...
  useEffect(() => {
    if (trade && trade.products) {
      ...
      if (origin_country != "All") {
        trade.products
          .filter((item: Product) => item.origin_country === origin_country)
          .forEach((product: Product) => {
            hsCodeFrequency[product.hs_code] =
              (hsCodeFrequency[product.hs_code] || 0) + 1
          })
        setHsCodeCounts(hsCodeFrequency)
        // console.log("filtering..")
        const uniquehscode: any = [
          "All",
          ...new Set(
            trade.products
              .filter((item: Product) => item.origin_country === origin_country)
              .map((item: Product) => item.hs_code)
          ),
        ]
        ...
        const uniquehscode: any = [
          "All",
          ...new Set(trade.products.map((item: Product) => item.hs_code)),
        ]
        sethscode(uniquehscode)
      }
    }
  ...
  const handleCountryfilter = (country: string) => {
    setOrigin_country(country), setIsFilterActive(true)

    country === "All" ? setIsFilterActive(false) : null
  }

  const handleDestinationCountryFilter = (country: string) => {
    setDestination_country(country);
  ...
  const handlehsCountfilter = (hscode: string) => {
    sethscode_filter(hscode), setIsFilterActive(true)
  ...
  const handlestateclick = (state: string, name: string) => {
    ...
    const citiesArray = country
      ? City.getCitiesOfState(selectedcountry, state).map((city) => city.name)
      : []
  ...
  // console.log("this is type---------------->", window.location.search)
  // console.log("this is type des---------------->", destinationCountryCounts)
  // console.log("this is type coun---------------->", countryCounts)

  // console.log("Import Or Export ----> ", typeQueryParam)
  ...
  return (
    <>
      ...
      <Accordion type="single" collapsible className="mt-4 w-full">
        <AccordionItem value="Date">
          ...
          >
            ...
            <input
              ...
              placeholder="01/30/2019-05/01/2023"
            />
          </AccordionTrigger>
        </AccordionItem>
        <AccordionItem value="port"></AccordionItem>
          ...
          <AccordionContent className="overflow-x-hidden">
            <ul className="ml-6 max-h-[150px] px-2 py-4 ">
              {Object.entries(hsCodeCounts).map(([hscount, count]) => (
                ...
                >
                  <div
                    className="flex justify-between text-[12px] uppercase text-dark-jungle-green"
                    onClick={() => handlehsCountfilter(hscount)}
                  >
                    <div>{hscount}</div>
                    <div>({count})</div>
                  </div>
        ...
        {type.type === "trade" && typeQueryParam === "import" ? (
          <>
        ...
        ) : (type.type === "trade" && typeQueryParam === "export") ? (
          <>
            <AccordionItem value="country">
              <AccordionTrigger
                icon={false}
              ...
              <AccordionContent className="overflow-x-hidden">
                <ul className="max-h-[150px] px-2 py-4 ">
                  {Object.entries(destinationCountryCounts).map(([country, count]) => (
                    <ListItem
                      key={`${country}`}
                      className="my-[2px] ml-6 px-0 hover:bg-transparent"
                    >
                      <div
                        className="flex justify-between text-[12px] uppercase text-dark-jungle-green"
                        onClick={() => handleDestinationCountryFilter(country)}
                      >
                        <div>{country}</div>
                        <div>({count})</div>
                      </div>
                    ...
                    states.map((state: { name: string; isoCode: string }) => (
                      ...
                      >
                        <div
                          className="flex justify-between text-[12px] uppercase text-dark-jungle-green"
                          onClick={() =>
                            handlestateclick(state.isoCode, state.name)
                          }
                        >
                          <div>{state.name}</div>
                        </div>
            ...
            <AccordionItem value="Range">
              ...
              <AccordionContent>
                <div className=" p-2" style={{ height: "200px" }}>
                  <div className="mb-8">
                    <SliderInput title="Net Weight Kg" sub="" />
                  </div>
                  <div className="mb-8">
                    <SliderInput title="Total Value USD" sub="(In Thousand)" />
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
