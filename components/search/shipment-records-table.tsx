...
export default function ShipmentRecordsTable({
  ...
  // console.log("type of Query -----> ", typeQueryParam)
  ...
  useEffect(() => {
    ...
    if (isFilterActive && origin_country && origin_country !== "All") {
      newData = newData.filter(
        (item: Product) => item.origin_country === origin_country
      )
    } else if (
      isFilterActive &&
      destination_country &&
      ...
      // Apply destination country filter if filter is active and destination_country is specified
      newData = newData.filter(
        (item: Product) => item.destination_country === destination_country
      ) 
    }
    ...
    if (isFilterActive && hscode_filter && hscode_filter !== "All") {
      newData = newData.filter(
        (item: Product) => item.hs_code === hscode_filter
      )
    }
  ...
  const columns = [
    ...
    "Importer Name",
    "Exporter Name",
    typeQueryParam === "import" ? "Origin Country" : "Destination Country",
    "Quantity",
    "Unit",
    "Net Weight[Kg]",
    "Total Value[usd]",
  ...
  return (
    <div className="mt-4 max-w-[100vw] ">
      ...
      {trade ? (
        isError(trade) &&
        trade.message === "Request failed with status code 400" ? (
          <div>
            <p>code : {trade.code}</p>
          </div>
        ) : (
            ...
            <TableBody>
              {filteredData.map((item: any, idx: any) => (
                <TableRow key={`${item.hs_code}_${idx}`}>
                  ...
                  <TableCell className="p-2 px-3">
                    <a>{item.destination_country || item.origin_country}</a>
                  </TableCell>
